...
public class Candidate360Test {
	...
	public void testGetCandidateSet() throws Exception {
		...
		XmlDiffUtils diff = new XmlDiffUtils().ignoreAttribute("xml:base").ignoreTag("updated").ignoreTag("id")
				.diff(response.getContent(), dest);
		assertNull(diff.getDiff()); //()內容是null->success
	...
	public void testSearchCandidate() throws Exception {
		...
		System.out.println("HIIII"+response);
		System.out.println("\n");
	}
	...
	public void testAptitudeTestResultList() throws Exception {
		...
		XmlDiffUtils diff = new XmlDiffUtils().ignoreTag("transactionid").ignoreTag("timestamp").diff(response.getContent(),
